...
class AnalogueClock3 extends JFrame implements ItemListener, Runnable {
	/*
	 * Image memoryimage; //더블 버퍼링 기법을 위한 메모리상에 화면 사이즈만큼의 이미지 버퍼를 생성 Graphics
	 * mgc;//생성한 이미지 버퍼에 대한 그래픽스 객체를 가져온다.
	 */
...
//메뉴 셋팅//
	...
	public void init() {
		...
		JPanel brazil_panel = new JPanel();
		brazil_panel.setBounds(500, 422, 50, 50);
		add(brazil_panel);
		brazil_panel.setLayout(null);
		...
		brazilbtn.setBounds(0, 0, 50, 50);
		brazil_panel.add(brazilbtn);
	...
	public void paint(Graphics g) {
		/*
		 * Image memoryimage = createImage(500, 500);//더블 버퍼링 기법을 위한 메모리상에 화면 사이즈만큼의 이미지
		 * 버퍼를 생성 Graphics mgc = memoryimage.getGraphics();//생성한 이미지 버퍼에 대한 그래픽스 객체를
		 * 가져온다.
		 * 
		 * mgc.drawImage(img,9,51,this); mgc.clearRect(9 ,51,500,500);
